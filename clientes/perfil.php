<?php  
	...
	if (isset($_POST['guardar'])) {
		...
		if (sizeof($validation_errors) == 0) {
			$admin->actualizarPerfil($nombre, $apaterno, $amaterno, $domicilio, $new_email, $old_email, $id_usuario);
		}else{
	...
	if (isset($_POST['nueva_contrasena'])) {
		...
		if ($contrasena == $contrasena2) {
			if ($admin->cambiar_contrasena($contrasena, $contrasena2, $id_usuario)) {
		...
		<div class="row vdivide">
			<div class="col-md-7">
				...
				<form class="form-group" method="post" action="perfil.php">
					<label>Nombre</label>
					<input class="form-control mb-2" type="text" placeholder="<?php echo $cliente_data['nombre']?>" value="<?php echo $cliente_data['nombre']?>" name="nombre">

					<label>Apellido Paterno</label>
					<input class="form-control mb-2" type="text" placeholder="<?php echo $cliente_data['apaterno']?>" value="<?php echo $cliente_data['apaterno']?>" name="apaterno">

					<label>Apellido Materno</label>
					<input class="form-control mb-2" type="text" placeholder="<?php echo $cliente_data['amaterno']?>" value="<?php echo $cliente_data['amaterno']?>" name="amaterno">

					<label>Domicilio</label>
					<input class="form-control mb-2" type="text" placeholder="<?php echo $cliente_data['domicilio']?>" value="<?php echo $cliente_data['domicilio']?>" name="domicilio">

					<label>Email</label>
					<input class="form-control mb-2" type="email" placeholder="<?php echo $cliente_data['email']?>" value="<?php echo $cliente_data['email']?>" name="email">
			...
			<div class="col-md-5">
				<form class="form-group" method="post" action="perfil.php">
				    ...
				    <input class="btn btn-primary" type="submit" value="Establecer nueva contraseña" name="nueva_contrasena">
				</form>
				<?php
